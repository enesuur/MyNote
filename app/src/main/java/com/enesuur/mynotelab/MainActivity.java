...
public class MainActivity extends AppCompatActivity implements NoteFragment.onNoteSelectedListener {
    ...
    public ArrayList<Note> retrieveNotes(){
        ...
        for (File file : files){
            ...
            note.setHeader(header);notes.add(note);
        }
        return notes;
    }
    ...
    private void saveContent(Note note, String content) {
        ...
        String header = content.length() < 30 ? content : content.substring(0,30);note.setHeader(header.replaceAll("\n", " "));
